      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-4">
            ...
            <button type="button" className="inline-flex items-center gap-1.5 text-xs font-medium text-[var(--color-primary)] hover:underline mt-2">
          ...
          <div className="space-y-3">
            {ZAPIER_EVENTS.map((evt) => (
              <label key={evt.id} className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-sm text-[var(--color-text-body)]">{evt.label}</span>
              </label>
            ))}
          </div>
